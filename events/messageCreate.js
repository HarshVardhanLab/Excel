                ...
                if (!client.config.owner.includes(message.author.id) && !uprem && !sprem) {
                    const row = new ActionRowBuilder().addComponents(
                        new ButtonBuilder()
                            .setLabel('Premium')
                            .setStyle(ButtonStyle.Link)
                            .setEmoji('<:prime:1374601389698846761>')
                            .setURL('https://discord.gg/ExcelBot'),
                    )
                    const embeds = client.util.embed()
                    embeds
                        .setDescription(
                            '[Click here to buy Premium](https://discord.gg/ExcelBot) so that you can use this command.'
                        )
                        .setColor(client.color)
                    return message.channel.send({
